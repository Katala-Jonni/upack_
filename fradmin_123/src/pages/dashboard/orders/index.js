...
const applyFilters = (orders, filters) => orders.filter((order) => {
  if (filters.query) {
    ...
    // name, email, etc.
    const containsQuery = order.orderNumber.toLowerCase().includes(filters.query.toLowerCase());
  ...
  if (typeof filters.status !== 'undefined') {
    const statusMatched = order.stage.status === filters.status;
...
const OrderList = () => {
  ...
  const { loadOrder, loadStage, endOrder } = useAction();
  const { items } = useSelector(({ order }) => order);
  const { items: stagesList, tabs: tabsList } = useSelector(({ stage }) => stage);
  ...
  if (!orders.length && !stages.length && !tabs.length) return null;
