...
const DeliveryAddress = ({
  ...
  const handleAddNewAddress = address => {
    setAddressList(state => [...state, {
      ...address,
      id: Date.now()
    }]);
    console.log('address', address);
    handleFieldValueChange(address.street2, 'address');
    handleFieldValueChange(address.street2, 'street2');
    handleFieldValueChange(address.name, 'name');
    handleFieldValueChange(address.email, 'email');
    handleFieldValueChange(address.phone, 'phone');
  };

  const handleDeleteAddress = addressId => {
    console.log('clickHandleDeleteAddress');
    handleFieldValueChange('', 'address');
    setAddressList(state => state.filter(item => item.id !== addressId));
  };

  // console.log('DeliveryAddressErrors.address', errors.address);

  const handleEditAddress = (addressId, data) => {
    setAddressList(state => {
      return state.map(item => {
        if (item.id === addressId) return {
          ...data
        }; else return item;
      });
    });
  ...
  return <Card sx={{
    ...
    <FlexBetween mb={4}>
      <Heading number={2} title="Укажите контактную информацию" mb={0}/>
    ...
    <Grid container spacing={3}>
      {addressList.map((item, ind) => <Grid item md={4} sm={6} xs={12} key={ind}>
        {/*<Card onClick={() => handleFieldValueChange(item.street2, 'address')} sx={{*/}
        <Card sx={{
          padding: 2,
          boxShadow: 'none',
          cursor: 'pointer',
          ...
          borderColor: item.street2 === values.address ? 'primary.main' : 'transparent'
        }}>
          ...
          <Paragraph color="grey.700">{item.email}</Paragraph>
        </Card>
      </Grid>)}
    </Grid>
    ...
    {editAddressId ? <EditAddressForm handleEditAddress={handleEditAddress} active={editAddressId ? true : false}
                                      changeEditAddressId={changeEditAddressId}
                                      address={addressList.find(item => item.id === editAddressId)}/> : null}
    {!values.address || errors.address ? <Paragraph color="#ef5350">{errors.address}</Paragraph> : null}
  </Card>;
};
