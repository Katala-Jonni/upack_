...
// ==============================================================
const ContactInfo = ({ values, handleFieldValueChange }) => {
  ...
  const handleAddNewAddress = address => {
    console.log('handleAddNewAddress', address);
    // const data = {
    //   ...address
    // };
  ...
  const handleEditAddress = (addressId, data) => {
    ...
    // Object
    //   .keys(data)
    //   .forEach(el => handleFieldValueChange(data[el], el));
  ...
  return <Card sx={{
    ...
    <FlexBetween mb={4}>
      <Heading number={1} title={addressList.length > 1 ? 'Укажите организацию' : 'Ваша организация'} mb={0}/>
    </FlexBetween>
    <FlexBetween mb={4}>
      {addressList.length < 1 ? <NewAddressForm handleAddNewAddress={handleAddNewAddress}/> : null}
    </FlexBetween>
...
const DUMMY_ADDRESS_LIST = [
  ...
  //   name: "ООО ЮПАК",
  //   phone: "+79210103525",
  //   street2: "Полный адрес Доставки",
  //   street1: "Наименование точки"
  // },
  // {
  //   id: 2,
  //   name: "Office",
