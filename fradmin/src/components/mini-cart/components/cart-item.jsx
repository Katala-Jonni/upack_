...
export default function MiniCartItem({
  ...
  return <FlexBox py={2} px={2.5} key={item.id} alignItems="center" borderBottom="1px solid" borderColor="divider">
      <FlexBox alignItems="center" flexDirection="column">
        <Button size="small" color="primary" variant="outlined" onClick={handleCartAmountChange(item.qty + item.count, item)} sx={{
        height: 28,
        width: 28,
        borderRadius: 50
      }}>
        ...
        <Button size="small" color="primary" variant="outlined" disabled={item.qty === 1} onClick={handleCartAmountChange(item.qty - item.count, item)} sx={{
        height: 28,
        width: 28,
        borderRadius: 50
      }}>
